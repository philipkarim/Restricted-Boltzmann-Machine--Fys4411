...
void SGD::SGDOptimize(vec parameters_derivative){
    ...
    for (int i=0; i<numberOfVN; i++){
        a_visible[i] = a_visible[i] - m_eta*parameters_derivative[i];
    }
    ...
    for (int j=0; j<numberOfHN; j++){
        ...
        b_hidden[j] = b_hidden[j] - m_eta*parameters_derivative[index_b];
    ...
    for (int k=0; k<numberOfVN; k++){
        for (int l=0; l<numberOfHN; l++){
            w_weight(k, l) = w_weight(k, l) - m_eta*parameters_derivative[index_w];
            index_w++;
